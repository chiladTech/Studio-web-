...
import {
  Users, UserPlus, CheckCircle, Loader2, ShieldCheck, Trash2, Sliders, CheckSquare, Square, Save, Eye, X, Lock, Check
} from 'lucide-react';
  ...
  const handleSaveUserPermissions = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage(`Error: ${err.error || 'Failed to save permissions'}`);
      }
    } catch {
      setMessage('Network error occurred while saving user permissions.');
    } finally {
  ...
  const handleDeleteUser = async (targetUser: UserItem) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        setMessage(`Error: ${err.error || 'Failed to delete user'}`);
      }
    } catch (e) {
      setMessage('Network error occurred while deleting user.');
    }
  };
        ...
        <main className="flex-1 p-8">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-xl font-bold text-neutral-900 flex items-center gap-2">
                ...
                User Management & Individual Sidebar Permissions
              </h1>
              <p className="text-xs text-neutral-500 mt-1">
                Configure unique custom sidebar permissions for each administrator by checking/unchecking specific pages
              </p>
            </div>
          ...
          {message && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-900 rounded-xl text-sm flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-[#6a1b2a] shrink-0" />
              <span>{message}</span>
            </div>
          )}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-neutral-50 border-b border-neutral-200 text-[11px] font-bold text-neutral-500 uppercase tracking-wider">
                  ...
                  {isSystemAdmin && <th className="p-4 text-right">Sidebar & Actions</th>}
                </tr>
              </thead>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {!isTargetSystemAdmin ? (
                              ...
                              >
                                ...
                                <span>Sidebar Pages</span>
                              </button>
                            ) : (
                              <span className="text-[11px] text-neutral-400 italic px-2">Full System Access</span>
                            )}
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setUserAllowedPages(['/admin/stories', '/admin/portfolio', '/admin/media'])}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[50vh] overflow-y-auto p-1">
                      {AVAILABLE_PAGES.map((page) => {
                        const isChecked = userAllowedPages.includes(page.href);
                        return (
                          <div
                            key={page.href}
                            ...
                            className={`p-3 rounded-xl border cursor-pointer transition-all flex items-start gap-2.5 select-none ${
                              isChecked
                                ? 'bg-[#fcf7f8] border-[#6a1b2a] shadow-xs'
                                : 'bg-white border-neutral-200 hover:border-neutral-300 opacity-60'
                            }`}
                          >
                            <div className="mt-0.5 shrink-0 text-[#6a1b2a]">
                              {isChecked ? (
                                <CheckSquare className="w-4 h-4 fill-[#6a1b2a] text-white" />
                              ) : (
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-neutral-900">{page.label}</span>
                                <span className="text-[9px] font-bold text-neutral-400 uppercase">{page.category}</span>
                              </div>
                              <p className="text-[11px] text-neutral-500 mt-0.5 leading-tight">{page.desc}</p>
                              <span className="text-[10px] text-neutral-400 font-mono">{page.href}</span>
                            </div>
                          </div>
                        );
                      })}
