...
export default function AdminMediaPage() {
  ...
  const handleDelete = async (asset: any) => {
    if (
      !confirm(
        `Are you sure you want to permanently delete "${asset.originalName || asset.filename}"? This will remove it from Vercel Blob and database.`
      )
    ) {
      return;
    }
  ...
  const handleUploadComplete = async (result: UploadResult) => {
    setMessage(`✅ Uploaded "${result.originalName}" to Vercel Blob!`);
    await loadMediaAssets();
  };
    ...
    <div className="min-h-screen bg-neutral-100 flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 md:p-8 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-neutral-900 flex items-center gap-2.5">
                ...
                Media Library & Vercel Blob Storage
              </h1>
              <p className="text-xs text-neutral-500 mt-1">
                Upload and manage high-resolution studio photos and cinematic video reels delivered via Vercel Blob CDN.
              </p>
            </div>
          ...
          {/* Collapsible Direct Blob Uploader Panel */}
          {showUploader && (
            <div className="bg-white rounded-2xl border border-neutral-200 p-6 mb-8 shadow-sm">
              ...
              <MediaUploader
                ...
                description="Files stream directly to Vercel Blob storage. No 4.5MB limit, no Base64 database bloat."
          ...
          {filteredAssets.length === 0 ? (
            <div className="bg-white rounded-2xl border border-dashed border-neutral-300 p-12 text-center">
              ...
              <p className="text-xs text-neutral-400 mt-1">
                Upload photos or video reels above to add them to your Vercel Blob library.
              </p>
            </div>
          ) : (
